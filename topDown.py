...
def tdc(input: list, dim: int, prettyPrintLocation: list):
    ...
    for i in range(dim, NUM_DIMS):
        ...
        for j in range(C): 
            ...
            if (c >= MINSUP):
                ...
                newLocation[i] = '*'
                tdc([table[idx] for idx in range(k, k+c)], i+1, newLocation)
            k+= c
        prettyPrintLocation[i] = '0'
    return
...
def main():
    ...
    tdc(data, 0, ['0' for i in range(NUM_DIMS)])
    return 0
